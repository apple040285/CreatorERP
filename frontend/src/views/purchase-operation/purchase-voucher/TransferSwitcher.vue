<template>
  <div>
    <b-input-group>
      ...
      <b-form-input
        disabled
        :value="showData.transfer ? showData.transfer.procurement_order_no : showData.transfer_order_no"
      />
...
export default {
  ...
  setup(props, { root, emit }) {
    ...
    const selectTransfer = id => {
      axios.get(`/procurement-orders/${id}`)
        .then(response => {
          const data = response.data

          // 轉單
          root.$set(props.showData, 'transfer_type', 'App\\Models\\ProcurementOrder');
          root.$set(props.showData, 'transfer_order_no', data.procurement_order_no);
